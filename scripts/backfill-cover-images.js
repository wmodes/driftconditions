...
const stopPhaseIdx  = args.indexOf('--stop-phase');
const STOP_PHASE    = stopPhaseIdx  !== -1 ? parseInt(args[stopPhaseIdx  + 1], 10) : 3;
...
async function main() {
  ...
  // Build the WHERE clause based on which clips to target:
  ...
  // MySQL requires LIMIT before OFFSET; 18446744073709551615 is the max BIGINT UNSIGNED (effectively unlimited)
  ...
  // Clips that need Haiku to generate an iTunes search query
  const needsQuery = [];
  ...
  if (STOP_PHASE < 2 || needsQuery.length === 0) { await done(); return; }
  ...
  // Clips that failed Phase 2 and fall through to Phase 3 image search
  ...
  if (START_PHASE <= 2) {
    ...
    if (needsAlbumSearch.length === 0) { await done(); return; }
  ...
  if (STOP_PHASE < 3 || needsImageSearch.length === 0) { await done(); return; }
  ...
  if (needsImageSelect.length === 0) { await done(); return; }
...
// When RETRY_NOT_FOUND is set, strips the old image-not-found tag first to avoid duplicates.
