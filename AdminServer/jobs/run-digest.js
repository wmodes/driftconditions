...
//   cd /path/to/driftconditions/AdminServer && node jobs/run-digest.js
//
// Cron (daily at 9am):
//   0 9 * * * cd /path/to/driftconditions/AdminServer && node jobs/run-digest.js >> ../logs/digest.log 2>&1

const path = require('path');

require('dotenv').config(); // loads AdminServer/.env when cwd is AdminServer/

const { runDigest } = require(path.join(__dirname, '../utils/digestRunner'));
