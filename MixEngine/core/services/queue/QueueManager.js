...
class MixQueue {
  ...
  async getNextMixID() {
    logger.info('QueueManager:getNextMixID(): Getting the next mixID');
    try {
      ...
      logger.debug(`QueueManager:getNextMixID(): Next mixID: ${nextMixID}`);
      return nextMixID;
    } catch (error) {
      logger.error('QueueManager:getNextMixID(): Error getting next mixID', error);
  ...
  async createMixQueueEntry(recipe, mixDetails) {
    logger.info('QueueManager:createMixQueueEntry(): Creating database entry for the mix');
    try {
      ...
      logger.debug(`QueueManager:createMixQueueEntry(): queryStr: ${queryStr}`);
      logger.debug(`QueueManager:createMixQueueEntry(): queryValues: ${JSON.stringify(queryValues, null, 2)}`);
      const [result] = await db.execute(queryStr, queryValues);
      // Log the newly created mixID
      logger.debug(`QueueManager:createMixQueueEntry(): New mix created with mixID: ${result.insertId}`);
      return result.insertId;
    } catch (error) {
      logger.error('QueueManager:createMixQueueEntry(): Error creating mix entry', error);
      throw new Error('Failed to create mix entry');
    }
  }

  async getNumberOfMixesInQueue() {
    logger.info('QueueManager:getNumberOfMixesInQueue(): Getting the number of mixes in the queue');
    try {
      ...
      logger.debug(`QueueManager:getNumberOfMixesInQueue(): Number of mixes in the queue: ${mixCount}`);
      return mixCount;
    } catch (error) {
      logger.error('QueueManager:getNumberOfMixesInQueue(): Error getting number of mixes in queue', error);
